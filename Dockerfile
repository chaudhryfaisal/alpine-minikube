...
RUN apk --update --no-cache add docker sudo bash && apk upgrade --update && \
...
git clone --depth 1 https://github.com/kubernetes/minikube && cd minikube && \
...
apk del .build-dependencies && apk --no-cache add curl
